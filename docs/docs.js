...
document.querySelectorAll('.show-code').forEach((elem) => {
    ...
    var badge = (elem.hasAttribute('data-badge') ? '<'+(elem.hasAttribute('data-badge-link') ? 'a href="'+elem.getAttribute('data-badge-link')+'"' : 'span')+' class="badge base outline xs ml-1">'+elem.getAttribute('data-badge')+(elem.hasAttribute('data-badge-link') ? '</a>' : '</span>') : '');
    ...
    var titleNote = (elem.hasAttribute('data-title-note') ? '<div class="note xs my-1 color-base">'+elem.getAttribute('data-title-note')+'</div>' : '');
